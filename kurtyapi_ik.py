...
# Veri Tabanı Simülasyonu (Gerçekte Cloud DB'ye bağlanacak)
...
tab1, tab2 = st.tabs(["📲 Personel Talep Ekranı", "⚙️ Yönetici Onay Paneli (0533 268 02 61)"])
...
with tab1:
    ...
    with st.form("izin_formu"):
        ...
        gonder = st.form_submit_button("Talebi Gönder ve Yöneticiye SMS At")
        ...
        if gonder:
            if ad:
                ...
                st.success(f"Talep alındı! Onay yetkilisine (0533 268 02 61) otomatik SMS gönderiliyor...")
            else:
...
with tab2:
    ...
    if not st.session_state.izin_talepleri.empty:
        onaylanacak = st.selectbox("İşlem yapılacak personeli seçin:", st.session_state.izin_talepleri[st.session_state.izin_talepleri['Durum'] == '⏳ Bekliyor']['Personel Adı'].tolist() if not st.session_state.izin_talepleri[st.session_state.izin_talepleri['Durum'] == '⏳ Bekliyor'].empty else ["Talep Yok"])
        
        if onaylanacak != "Talep Yok":
